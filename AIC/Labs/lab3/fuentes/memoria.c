...
valor_t mem_lee_region_datos(
    ...
    switch (tipo) {
        ...
        case t_byte:
        case t_char:
            if (!esta_alineada_dir_byte(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de datos (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
            resultado.int_d = mm->datos[r].m_byte[indice_byte(offset)];
            break;
        case t_half:
            if (!esta_alineada_dir_half(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de daots (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
            resultado.int_d = mm->datos[r].m_half[indice_half(offset)];
            break;
        case t_word:
            if (!esta_alineada_dir_word(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de datos (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
            ...
            break;
        case t_dword:
            if (!esta_alineada_dir_dword(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de datos (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
            ...
            break;
        case t_float:
            if (!esta_alineada_dir_float(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de datos (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
            ...
            break;
        case t_double:
            if (!esta_alineada_dir_double(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de datos (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
            ...
            break;
        case t_float_ps:
            if (!esta_alineada_dir_float_ps(offset)) {
                fprintf(stderr, "ERROR: Lectura desalineada en memoria de daots (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
...
valor_t _lee_mem_datos(
    ...
    if (es_region_datos(r)) {
        if ((direccion>=inicio_region(mm, r)) && (direccion<=final_region(mm, r))) {
...
void mem_escribe_region_datos(
    ...
    switch (tipo_dato) {
        ...
        case t_byte:
            if (!esta_alineada_dir_byte(offset)) {
                fprintf(stderr, "ERROR: Escritura desalineada en memoria de datos (dir: " F_DWORD ")\n", inicio_region(mm,r)+offset);
                exit(1);
            }
...
void escribe_mem_datos(
    ...
    if (es_region_datos(r)) {
        if ((direccion>=inicio_region(mm, r)) && (direccion<=final_region(mm, r))) {
            ...
            fprintf(stderr, "ERROR: Escritura fuera del rango de memoria de datos (dir: " F_DWORD ")\n\ten %s:%d\n ",
